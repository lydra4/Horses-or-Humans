...
def mlflow_init(args, run_name, setup_mlflow=False, autolog=False):
    ...
    if setup_mlflow:
        try:
            ...
            run_name = args.get("mlflow_run_name", run_name)
            if "MLFLOW_HPTUNING_TAG" in os.environ:
                run_name += "-hp"

            run_name += f"-{time.time():.0f}"
            ...
            logger.error("MLflow initialisation has failed. Error: %s", str(e))
